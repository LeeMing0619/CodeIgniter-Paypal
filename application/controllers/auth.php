...
',//'rps_outgoings@hotmail.com',
        ...
        public function changeEmail() {
            ...
            $newpwd = $this->input->post('newpwd');
            $useremail = $this->session->userdata['user_email'];
            $isMarket = $this->input->post('market');
            
            $returnVal = $this->Auth_model->updateEmail($useremail, $newemail, $isMarket);
            $this->Auth_model->resetPassword($useremail, $newpwd);
            if ($returnVal != 'fail')
            {
        ...
        public function login(){
            if ($this->input->is_ajax_request()) {
                ...
                if($this->Auth_model->login($username, $password)){
                    ...
                    //Game History
                    $wins = $this->Admin_model->getWinsHistory($useremail);
                    $losses = $this->Admin_model->getLooseHistory($useremail);
                    $draws = $this->Admin_model->getDrawsHistory($useremail);
                    $poeples = $this->Admin_model->getWithPeoples($useremail);
                    
                    $cnt['wins_cnt'] = $wins->num_rows();
                    $cnt['losses_cnt'] = $losses->num_rows();
                    $cnt['draws_cnt'] = $draws->num_rows();
                    $cnt['game_cnt'] = $poeples->num_rows();
                    
                    $this->session->set_userdata('games_cnt', $cnt);
