...
class Admin extends CI_Controller {
        ...
        if ($family['betamount'] == '' && $family['gamemode'] == 0) 
        {
            ...
            if ($paymentMethod == 0) { //by with balance
                ...
                if ($res == 'fail') {
                    ...
                    $this->show_question($family['brain_game_type']);
                }
    ...
    public function show_question($type) {
        if ($type == 0) {
            ...
            $res_answer_id = $this->Admin_model->getAnswerIDForNomal($res['answer_id']);
            $data['answers'] = $res_answer_id;
            ...
            $res_answer_id = $this->Admin_model->getAnswerIDForHard($res['answer_id']);
            $data['answers'] = $res_answer_id;
    ...
    public function show_question_join($type) {
        if ($type == 0) {
            $res = $this->Admin_model->getQuestions_join();
    ...
    // create brain game and select answer
    public function getAnswerNewQuestion() {
        $q_id = $this->input->post('q_id');
        $a_id = $this->input->post('an_id');
    ...
    public function showLogList() {
        ...
        $chatPeoples['peoples'] = $this->Chat_model->getPeoples($email);
        
        $this->load->view('admin/show_log_activities', $chatPeoples);
    ...
    public function showOpenRooms() {
        ...
        if ($query->num_rows() < 1) 
            echo "<h3 style='text-align: right;'>There isn't Open Rooms.</h3>";
        foreach ($query->result() as $val){
            ...
            if($val->game_kind==0)
                ...
                if ($val->potential_return > 0)
                    $winnings = $val->potential_return;
                else
                    $winnings = $bet_amount;
            } else if ($val->game_kind == 2) {
                if ($val->potential_return > 0)
                    ...
                    $winnings = $bet_amount + $val->potential_return;
            } else if ($val->game_kind == 3) {
                if ($val->potential_return > 0)
                    $winnings = $val->potential_return;
                else
                    $winnings = $bet_amount;
            ...
            echo '<tr class="'.$val->id.'"><td>'.$RoomID.'</td><td>&pound;'.$pr.'</td><td>&pound;'.$winnings.'</td><td class="'.$val->bet_amount.'"><button type="button" onclick="javascript:cross_win($(this) , '.$val->game_kind.');">END</button></td></tr>';
        }
        exit;    
    }
    ...
    public function showWinsHistory() {
        ...
        foreach ($query->result() as $val){
            ...
            echo '<tr class="'.$val->id.'"><td>'.$RoomID.'</td><td>'.$val->fname.'</td><td>'.$val->uname.'</td><td>&pound;'.$bet_amount.'</td><td>'.$val->end_date.'</td></tr>';
        }
        exit;    
    }
    ...
    public function showDrawsHistory() {
        ...
        foreach ($query->result() as $val){
            ...
            echo '<tr class="'.$val->id.'"><td>'.$RoomID.'</td><td>'.$val->fname.'</td><td>'.$val->uname.'</td><td>&pound;'.$val->bet_amount.'</td><td>'.$val->end_date.'</td></tr>';
        }
        exit;    
    }
    
    public function memberListLoad() {
        $sql = "select m.id,CONCAT('Rock', m.id)as member_id, m.fname, m.user1,  m.status, m.bet_amount, m.password, m.game_kind, m.potential_return , a.photo, a.bio
                from member m join auth a where a.email=m.user1 and (m.win='' or m.win='incorrect') order by m.id desc";
    ...
    public function recentListLoad() {
        ...
        $sql = "select m.id, m.fname, m.user1, a.uname as user1_name, a.photo as photo1, a.bio as bio1, m.user2, b.uname as user2_name,b.photo as photo2, 
                b.bio as bio2, m.status, m.bet_amount, m.password, m.game_kind, m.win, TIMESTAMPDIFF(MINUTE, m.end_date, NOW()) as minutes,  m.box_prize, SUBSTRING_INDEX(m.guess_log, ',', -1) as guess
        ...
        if($is_connecting > 0) {
            ...
            if ($paymentMethod == 0) {
                ...
                if ($res == 'fail') {
                    ...
                    $type = $this->Admin_model->getBrainGameDiff($id_value);
                    $this->show_question_join($type);
                }
            } else {
                $user = $this->session->userdata['user_name'];
